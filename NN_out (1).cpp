...
//  friend class VectorD;
    ...
    public:
      ...
      int LoadWorkingSet(string file, int nInp, int nHid, int nOut);//gotta fix
...
int NN::LoadWorkingSet(string file,int nInp,int nHid , int nOut){
    ...
    for(int iim = 0; iim<nTrainingEntries; iim++) {
        ...
        while(std::getline(lineStream,cell,' ')) {
            ...
            } else {//doesn't reach answer parts
...
void NN::BackProp(){
    ...
    for(int i = 0; i < nOutputs; i++){//same size as nOutputs
        deltaOutput[i] = d_activation(netOutput[i]) * currentError[i];//δi = σ(z) · (1 − σ(z)) * (y-t)
        //z = netOutput for now, changes later
        d_biasOutput[i] = deltaOutput[i];//biases are changed 
		//derroe/dbj = δ derivative of error in respect to output is delta output slide 67
        for(int j = 0; j < nHiddenNeurons;j++){
			//output of the weights = δi * outputs 
			d_weightsOutput[i*nHiddenNeurons+j] = deltaOutput[i] * outHidden[j]; //changing the direvatives of the error in respect to the weightsOutput
			//vias biases * hidden outputs 
		}
    }//end of δi 
    //working on hidden layers start
        //working on the nhidden
    for(int x = 0; x < nHiddenNeurons; x++){//x is all hidden neutrons
		deltaHidden[x] = 0.0;//
		double sumOutput =0; //suming up ( Σ iδi· wouti,j)
        for(int i = 0; i < nOutputs; i++){
            //getting all z = netHidden all in respect to nHiddenN dw
           sumOutput += deltaOutput[i] * weightsOutput[x*nOutputs+i]; //  Σ (δ i * weights of output) 
			}
        double z = netHidden[x];//z is hidden net for that formula 
        deltaHidden[x] = sumOutput * d_activation(z); // since it sum output refreshes each time no need to make an array
        //δj (delta hidden) = Σ δi * derivative(z)
        d_biasHidden[x] = deltaHidden[x]; //tweaking biases of hidden neurons from the delta of the hidden, repeated what we did for the output
        //finally reaching the end (input)
        for(int i = 0; i < nInputs; i++){
			d_weightsHidden[x*nInputs+i] = deltaHidden[x] * currentInputs[i];
			//changing hiddens weighhts for the inputs (last step of propagation)
			//de/dweights = δj * inputs
		}
	}
}
...
void NN::PrintOutputs(){
    ...
    for (int out = 0 ; out < nOutputs ; out++){
        ...
        if(currentOutputs[out] > 0.5){//makes it easier to see
            imageN = out;
        }
    }
    cout<<endl;
    cout<<endl;
    cout<<"Computer says the image number is: "<< imageN<<endl;
    cout<<endl;
}
...
int main(){
    ...
    neuralNet.Train1();
    neuralNet.LoadWorkingSet("work.txt",64,128,8);//after training it will automatically load the working set, replacing all the variables of loa
